...
{
          cout<<mi_arreglo[i][j]<<"\t |\t ";
			promedios[i] += mi_arreglo[i][j];
        }
        promedios[i] = promedios[i] / m;
        cout << promedios[i]<< endl;
        promedio_global += promedios[i];
        
       
        }
//		cout << promedio_global;
		promedio_global = promedio_global / n;
		cout << endl;
		cout << "El promedio Global de los Alumnos es: ";
		cout << promedio_global << endl;
		cout << endl;
    ...
    case 'd':
//    	utilizacion de funcion sort para ordenar los resultados de mayor a menor
//    	utilizar un bucle for con la condicion que se ejecute a < 3, a 0, 1, 2

		
//    int array[5] = {3,5,7,6,1};
	int i,j;
